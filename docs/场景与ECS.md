...
```cpp
scene.SetEnvironment({.sunIntensity = 0.7f});   // 只作用于方向光，默认 1.0
```

`sunIntensity` 是场景级的太阳倍率，打包进帧块时只乘到方向光上——点光/聚光
各有自己的 `intensity`，场景想调暗它们就调它们自己。它缩放的是太阳的辐亮度
而不是下面的 `exposure`，所以调暗时光影位置不变、太阳颜色保留，云的受光也
跟着一起变（云和地面读的是同一盏灯）。

### 云层

云是**世界空间体积**，不是贴在天上的图画：噪声在采样点求值，所以云有近面、
有远面，摄像机可以飞进去，也可以飞到天气之上。整层由四个参数定形：

| 参数 | 作用 |
|---|---|
| `cloudAltitude` / `cloudThickness` | 层底高度与厚度（世界单位，默认 1200 / 1700） |
| `cloudWeatherScale` | 天气单元尺寸：决定"哪里有云"，是唯一按地貌尺度授权的量 |
| `cloudDetailScale` / `cloudDetailStrength` | 云缘侵蚀的尺寸与深度 |
| `cloudType` | 0 为层云片状、1 为积云塔状；逐天气单元变化 |

`cloudAltitude` 刻意压得低。眼睛判断"云远不远"靠的不是高度本身，而是云
**最早在哪个仰角出现**，也就是 `高度 / tan(仰角)`：层挂在五千米时只有接近
天顶的射线够得着，天空在视线真正停留的地方全是空的，天气被压成画面顶端的
一条边。几百米到一千米的层，就是站在它下面看到的同一片天气。

`cloudQuantity` 之外的三个步骤计数（`cloudSteps` 细步、`cloudCoarseSteps`
粗步、`cloudLightSteps` 向阳步）是这一层里唯一值得数的开销：粗步先用一档
噪声找出层里真正有云的那一段，细步只走那一段，向阳步再逐采样点向太阳累积
光学厚度。`cloudSteps = 0` 整个关闭云层。
