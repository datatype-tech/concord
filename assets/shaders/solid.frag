...
vec3 EvaluateLight(FrameLightData light, vec3 baseColor, vec3 normal, vec3 position,
                   vec3 viewDirection, float metallic, float roughness)
{
    ...
    if (light.positionType.w == 0.0) {
        ...
        if (light.positionType.w == 2.0) {
            vec3 fromLight = NormalizeOrUp(position - light.positionType.xyz);
            float cone = dot(fromLight, NormalizeOrUp(light.directionRange.xyz));
            if (cone < light.spotShadow.x) {
                return vec3(0.0);
            }
        }
    }
...
void main()
{
    ...
    if (useTiles) {
        ...
        if (tileX >= 128u || tileY >= 128u) {
            useTiles = false;
        } else {
            TileHeader header = tile.tiles[tileY * 128u + tileX];
            if (header.overflow != 0u) {
                useTiles = false;
            } else {
                uint count = min(header.count, 64u);
                for (uint j = 0u; j < count; ++j) {
                    uint index = tile.indices[header.offset + j];
                    if (index < lightCount) {
                        color += EvaluateLight(frame.lights[index], baseColor, normal,
                                               worldPosition, viewDirection, metallic, roughness);
                    }
                }
            }
        }
    ...
    if (!useTiles) {
        for (uint i = 0u; i < lightCount; ++i) {
            color += EvaluateLight(frame.lights[i], baseColor, normal, worldPosition,
