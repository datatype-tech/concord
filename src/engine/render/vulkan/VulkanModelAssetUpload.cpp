                       ...
                       VkBufferUsageFlags usage, VulkanBuffer& buffer)
{
    ...
    return CreateVulkanHostBuffer(context, size, usage, buffer) && UploadVector(buffer, values);
}
                            ...
                            VulkanModelAsset& output)
{
    ...
    if (!CreateVectorBuffer(context, data.vertices,
                            VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                            output.vertexBuffer) ||
        !CreateVectorBuffer(context, data.indices,
                            VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
                            output.indexBuffer) ||
        !CreateVectorBuffer(context, data.materials, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT,
