...
{
    if (context.device == VK_NULL_HANDLE || skinningLayout == VK_NULL_HANDLE) {
        return VK_NULL_HANDLE;
    }
    VkPushConstantRange range{};
    range.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    range.size = sizeof(VulkanSkinnedShadowPushConstants);
    VkPipelineLayoutCreateInfo info{};
    info.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
    info.setLayoutCount = 1;
    info.pSetLayouts = &skinningLayout;
                                                  ...
                                                  VkShaderModule vertex)
{
    ...
    VkVertexInputBindingDescription binding{0, sizeof(ModelVertex),
    ...
    input.vertexAttributeDescriptionCount = static_cast<u32>(attributes.size());
