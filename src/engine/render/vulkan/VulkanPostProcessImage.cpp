...
{
    const VkDevice device = slot.device != VK_NULL_HANDLE ? slot.device : context.device;
    if (device != VK_NULL_HANDLE && slot.view != VK_NULL_HANDLE) {
        vkDestroyImageView(device, slot.view, nullptr);
    }
    if (device != VK_NULL_HANDLE && slot.image != VK_NULL_HANDLE) {
                                  ...
                                  VulkanPostProcess& slot)
{
    ...
    VkResult result = vkCreateImage(context.device, &imageInfo, nullptr, &slot.image);
    if (result != VK_SUCCESS) {
        return VulkanFailed("vkCreateImage(post process)", result);
    }
    VkMemoryRequirements requirements{};
    vkGetImageMemoryRequirements(context.device, slot.image, &requirements);
    ...
    result = vkAllocateMemory(context.device, &allocation, nullptr, &slot.memory);
    if (result != VK_SUCCESS) {
        DestroyVulkanPostProcessImage(context, slot);
        return VulkanFailed("vkAllocateMemory(post process)", result);
    }
    result = vkBindImageMemory(context.device, slot.image, slot.memory, 0);
    if (result != VK_SUCCESS) {
        DestroyVulkanPostProcessImage(context, slot);
        return VulkanFailed("vkBindImageMemory(post process)", result);
    }
    ...
    result = vkCreateImageView(context.device, &viewInfo, nullptr, &slot.view);
    if (result != VK_SUCCESS) {
        DestroyVulkanPostProcessImage(context, slot);
        return VulkanFailed("vkCreateImageView(post process)", result);
    }
